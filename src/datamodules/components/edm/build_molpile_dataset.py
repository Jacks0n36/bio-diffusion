...
def extract_conformers(args):
    ...
    save_file = f"MolPILE_drugs_{'no_h_' if args.remove_h else ''}{args.conformations}"
    smiles_list_file = "MolPILE_drugs_smiles.txt"
    number_atoms_file = f"MolPILE_drugs_n_{'no_h_' if args.remove_h else ''}{args.conformations}"
    ...
    dataset_conformers = []
    mol_id = 0
    for i, drugs in enumerate(unpacker):
        log.info(f"Unpacking file {i}...")
        for smiles, all_info in drugs.items():
...
class MolPILETorchDataLoader(TorchDataLoader):
